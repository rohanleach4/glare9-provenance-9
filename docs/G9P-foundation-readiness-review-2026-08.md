...
## Deployment and assurance guidance

- The MySQL connector has passed an isolated local integration, CA-trusted TLS 1.3 and exact-table least-privilege qualification; repeat it in a customer's selected environment when that connector will be relied upon.
- The deployment owner should review and perform site-specific exercises for deployment, monitoring and incident runbooks.
- Exercise and review the selected custody profile, backups and transport identities on the intended host environment.
...
- Each deployment owner records its own reliance decision, claimed assurance level and acceptance of residual risks; the open-source project does not grant production approval.

This guidance is deliberately visible in `TODO.md` and `WHAT_YOU_NEED_FOR_COMMERCIAL_RELEASE.md`. Lack of funds for legal or external review does not prevent an Apache 2.0 source release, but no unavailable review will be implied or fabricated.
